...
DECLARE
    ...
    current_balance INT;
BEGIN
    -- กรองเงื่อนไข 1: มีการเปลี่ยนสถานะการจ่ายเงินเป็น 'paid' เท่านั้น
    -- (ป้องกันกรณี Update column อื่นแล้ว Trigger ทำงานฟรี)
    IF NEW.payment_status = 'paid' AND (OLD.payment_status IS DISTINCT FROM 'paid') THEN
        
        -- กรองเงื่อนไข 2: ยอดเงินจะต้องมากกว่า 0
        IF NEW.price_total_thb > 0 THEN
            
            -- ป้องกันแจกแต้มเบิ้ลขั้นเด็ดขาด (Idempotency Check)
            -- หาก Reference ของ Booking ID นี้เคยถูกแจกแต้มไปแล้วในประวัติ ให้ข้ามทันที
            IF EXISTS (
                SELECT 1 FROM public.point_history 
                WHERE reference_type = 'booking' AND reference_id = NEW.id
            ) THEN
                RETURN NEW;
            END IF;

            -- คำนวณแต้ม (100 บาท = 10 แต้ม) -> (ยอดเต็ม / 100) * 10
            -- ใช้ FLOOR ตัดเศษลง เช่น 1250 บาท = 120 แต้ม 
            earned_points := FLOOR(NEW.price_total_thb / 100) * 10;

            IF earned_points > 0 THEN
                -- อัปเดตตาราง profiles (เพิ่มแต้ม)
                -- การใช้ลูกศร = points + earned_points ช่วยป้องกัน Race Condition จาก Request ที่มาพร้อมกันได้
                UPDATE public.profiles
                SET points = points + earned_points
                WHERE user_id = NEW.user_id
                RETURNING points INTO current_balance;

                -- บันทึกเก็บเป็นประวัติว่าได้แต้มมาได้อย่างไร
                INSERT INTO public.point_history (
                    ...
                    current_balance,
                    'EARN_BOOKING',
                    'ได้รับแต้มจากการจองสนาม #' || NEW.id,
                    'booking',
                    NEW.id
                );
            END IF;
...
ALTER FUNCTION "public"."handle_booking_points_earn"() OWNER TO "postgres";
...
BEGIN
    ...
    SELECT id, point_cost, remaining_quantity, limit_per_user, status, end_date
    ...
    IF v_campaign_record.status != 'active' OR v_campaign_record.end_date < now() THEN
    ...
    -- จ. แจกคูปองเข้า Wallet พร้อบหักโควตาแคมเปญ
    INSERT INTO public.user_coupons (user_id, campaign_id, status)
...
ALTER FUNCTION "public"."redeem_points_for_campaign"("p_user_id" "text", "p_campaign_id" "uuid") OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."bookings" (
    ...
    "agreed_to_referral_terms" boolean DEFAULT false,
...
COMMENT ON COLUMN "public"."campaigns"."point_cost" IS 'จำนวนแต้มที่ต้องใช้ หากเป็น 0 คือแจกฟรีไม่ต้องใช้แต้ม';
...
CREATE TABLE IF NOT EXISTS "public"."referral_programs" (
    ...
    "require_term_consent" boolean DEFAULT false,
    "term_consent_message" "text"
);
...
CREATE TABLE IF NOT EXISTS "public"."system_settings" (
    "id" integer DEFAULT 1 NOT NULL,
    "stripe_deposit_amount" numeric DEFAULT 200,
    "updated_at" timestamp with time zone DEFAULT "now"(),
    "updated_by" "text",
    CONSTRAINT "single_row" CHECK (("id" = 1))
);
...
CREATE INDEX "idx_point_history_reference" ON "public"."point_history" USING "btree" ("reference_type", "reference_id");
...
CREATE OR REPLACE TRIGGER "trigger_earn_points_on_payment" AFTER UPDATE OF "payment_status" ON "public"."bookings" FOR EACH ROW EXECUTE FUNCTION "public"."handle_booking_points_earn"();
...
ALTER TABLE ONLY "public"."point_history"
    ADD CONSTRAINT "point_history_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("user_id") ON DELETE CASCADE;
...
CREATE POLICY "Enable ALL for authenticated users" ON "public"."system_settings" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "Users can view their own point history" ON "public"."point_history" FOR SELECT TO "authenticated" USING ((("auth"."uid"())::"text" = "user_id"));
...
GRANT ALL ON FUNCTION "public"."handle_booking_points_earn"() TO "anon";
GRANT ALL ON FUNCTION "public"."handle_booking_points_earn"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."handle_booking_points_earn"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."redeem_points_for_campaign"("p_user_id" "text", "p_campaign_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."redeem_points_for_campaign"("p_user_id" "text", "p_campaign_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."redeem_points_for_campaign"("p_user_id" "text", "p_campaign_id" "uuid") TO "service_role";
